...
$category_ids = isset($_GET['category_id']) ? $_GET['category_id'] : '';
$sql = "SELECT p.id, p.name, p.image, p.price, c.name AS 'Category', p.desscription, p.status FROM product p JOIN category c ON c.id = p.category_id WHERE p.category_id like '%$category_ids%' ";
